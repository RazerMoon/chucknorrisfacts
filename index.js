...
module.exports = class ChuckNorrisFacts extends Plugin {
  startPlugin () {
    this.run = () => get('https://api.chucknorris.io/jokes/random').execute().then((result) => this.showJoke(result.body)).catch((err) => this.warn(err));
  ...
  showJoke (joke) {
    powercord.api.notices.sendAnnouncement('chucknorrisfacts', {
      ...
      button: {
        onClick: () => {
          open(() => React.createElement(JokeModal, { joke }));
        },
        text: 'Show'
      }
    });
